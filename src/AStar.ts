class AStar {
    ...
    public search(): Boolean {
        var Search_node: Node_G = this._startNode;
        while (Search_node != this._endNode) {
            var startX: number = Math.max(0, Search_node.x - 1);
            var endX: number = Math.min(this._grid._numCols - 1, Search_node.x + 1);
            var startY: number = Math.max(0, Search_node.y - 1);
            ...
            for (var i: number = startX; i <= endX; i++) {
                for (var j: number = startY; j <= endY; j++) {
                    ...
                    var g: number = Search_node.g + cost * test.costMultiplier;
            ...
            Search_node = this._open.shift() as Node_G;
        }
        this.buildPath();
        return true;
    }
